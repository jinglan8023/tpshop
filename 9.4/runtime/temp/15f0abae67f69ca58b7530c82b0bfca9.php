<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:75:"/data/wwwroot/default/9.4/public/../application/index/view/car/carlist.html";i:1547812045;s:78:"/data/wwwroot/default/9.4/public/../application/index/view/layout/layout3.html";i:1539953286;s:75:"/data/wwwroot/default/9.4/public/../application/index/view/public/top3.html";i:1547861107;s:77:"/data/wwwroot/default/9.4/public/../application/index/view/public/footer.html";i:1537340772;}*/ ?>
...
<div class="i_bg">
    ...
    <div class="content mar_20">
        <table border="0" class="car_tab" style="width:1200px; margin-bottom:50px;" cellspacing="0" cellpadding="0">
            ...
            <?php
                $sum=0;
            if(is_array($car) || $car instanceof \think\Collection || $car instanceof \think\Paginator): $i = 0; $__LIST__ = $car;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
            <tr sku_id="<?php echo $v['sku_id']; ?>">
                <td>
                    <?php if(array_key_exists('car_id',$v)): ?>
                    <input name="car[]" type="checkbox" class="car_id" value="<?php echo $v['car_id']; ?>">
                    <?php else: ?>
                ...
                <td>
                    <div class="c_s_img">
                        <img src="__PUBLIC__/<?php echo $v['sku_img']; ?>" width="73" height="73" />
                    </div>
                    <a href="<?php echo url('Product/productDetail',['sku_id'=>$v['sku_id']]); ?>">
                        <!--<input type="hidden" value="<?php echo $v['sku_id']; ?>">-->
                        <?php echo $v['sku_name']; 
                        $money = ( $v['add_price'] - $v['sku_price'] );
                        if( $money > 0){
                        echo '<p style="color:#ff4e00">(比加入购物车时降价<?php echo $money; ?>元)</p>';
                        }
                        ?>

                    </a>
                </td>
                <td align="center">￥<span><?php echo formatMoney($v['sku_price']); ?></span></td>
                <td align="center">
                    <div class="c_num">
                        <input type="button" value="" class="car_btn_1" />
                        <input type="text" value="<?php echo $v['buy_number']; ?>"
                               <?php if($v['sku_stock'] > 200): ?>
                               stock="200"
                               <?php else: ?>
                        stock="<?php echo $v['sku_stock']; ?>"
                        <?php endif; ?>
                        name="" class="car_ipt" sku_id="<?php echo $v['sku_id']; ?>"/>
                ...
                <span>
                <!--<?php echo formatMoney($v['sku_price'] * $v['buy_number']); ?>-->
                <?php echo formatMoney($this_price); ?>
                </span>
                </td>
    ...
    <script type="text/javascript">
        ...
        function changeCarNumber(type,input_ele){
            ...
            index=layer.load(0,{shade:[0.5,'#ccc']});
            $.ajax({
                url:"<?php echo url('Car/carUpdate'); ?>",
        ...
        $('#confirm').click(function(){
            ...
            if(login==1){
                //登陆的情况下
                location.href="<?php echo url('Order/order'); ?>?car_id="+car_id;
            }else{
                //未登录的情况下
                alert('没有登录,请先登录');
                window.location.href="<?php echo url('Login/login'); ?>?refer="+window.location.href+"&car_id="+car_id;
                return false;
            }
        })
            ...
            $.post(
                    ...
                    function(res){
                        ...
                        if(res.code==1){
                            ...
                            if($('.car_id').length==0){
                                setTimeout(function(){
                                    location.href="<?php echo url('car/carlist'); ?>"},1000);
                            }
                        }
                    },
                    'json'
        ...
        $('.collection').click(function(){
            ...
            var sku_id=$('.car_ipt').attr('sku_id');
            $.post(
                    "<?php echo url('car/addcollection'); ?>",
